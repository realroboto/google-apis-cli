...
- **OAuth client próprio, modo produção** (não Testing) → sem a expiração de 7 dias do Testing mode. Google mantém gatilhos residuais (inatividade 6 meses, cap ~100 refresh tokens/client, troca de senha em tokens Gmail-scoped) — não aplicáveis ao uso normal desta CLI; o refresh automático (getAccessToken) reidrata transparente. Client type: Desktop app. `client_id`/`client_secret` embutidos no pacote (padrão de CLIs OAuth públicas: gcloud/gh/firebase-tools).
...
- **Executor rest (seam central, deep):** `execute(manifestRow, params, { tokenProvider, fetch })` — resolve `pathTemplate`+params → URL; monta headers (`Authorization: Bearer` via `tokenProvider` **injetado, não importado**; `x-goog-user-project`; `requiredHeaders` do row, ex. Ads `developer-token`/`login-customer-id`); envia via `fetch` (**também injetável** p/ teste); segue paginação (`nextPageToken`); aplica o `decoder` do row; formata `--json`/`--raw`. Generaliza o `api_call()` de `ga4-write.sh`. **Callers nunca constroem URL nem conhecem o token** — interface pequena, muito comportamento atrás dela. **Todo acesso a API passa por aqui.**
- **Decoder seam (Ads):** GA4/GSC/GTM/Indexing são JSON CRUD uniforme (decoder identidade); Ads `searchStream` retorna array de chunks streamados + exige `developer-token`. O executor fica uniforme; o `decoder` + `requiredHeaders` opcionais do manifesto absorvem a variância (stream-concat p/ Ads). Dois decoders = seam real, não hipotético.
- **Endpoints REST [V]:**
...
- **Config:** `developer-token` e `login-customer-id` do Ads via flags `--developer-token`/`--login-customer-id`, env (`GOOGLE_ADS_DEVELOPER_TOKEN`, `GOOGLE_ADS_LOGIN_CUSTOMER_ID`) ou `~/.config/gapi/config.json` (precedência flag → env → config). Quota project via env `GOOGLE_CLOUD_PROJECT` ou config.
...
- **Seam único = o executor `src/rest.ts`.** Testes de comando **injetam um `tokenProvider` fake + um `fetch` fake** (dependências aceitas, não criadas) e asseveram, por linha de manifesto: URL resolvida do `pathTemplate`+params, headers (Bearer, `developer-token`, `x-goog-user-project`), body, paginação (nextPageToken), decoder aplicado, `--json`/`--raw`, exit codes e mensagem em erro REST. Nenhum teste bate na rede real, e nenhum precisa de OAuth (token é injetado).
