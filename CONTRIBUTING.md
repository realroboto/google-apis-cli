...
- **scope** — one lowercase noun for the area touched, e.g. `auth`, `rest`, `gsc`, `manifest`, `deps`. Omit the `(...)` if none fits.
...
```
fix(rest): stop crash on empty page response
...
```
feat(auth)!: rename the login flag
