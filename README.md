...
- One dependency: `google-auth-library` (Google). Nothing third-party.
...
| Part | File | Function |
|---|---|---|
| **auth** | `src/auth.js` | `gapi auth login` gets one OAuth consent. It stores the refresh token in `~/.config/gapi/credentials.json` (mode 600). `getAccessToken()` refreshes the access token. Only part with real handlers. |
| **manifest** | `src/apis/*.js` | Each API is data, not code: `resource → verb → { httpMethod, pathTemplate, scopes, requiredHeaders?, decoder? }`. One file per surface. A glob finds them. No central registry. |
| **executor seam** | `src/rest.js` | `execute(manifestRow, params, { tokenProvider, fetch })` builds the URL, adds headers, follows pagination, applies the `decoder`, and formats `--json`/`--raw`. All network calls pass here. It is the one test seam. |

`bin/gapi.js` parses argv. The dispatch, `--help`, and URL resolution all come from the manifests.
...
- New API = one new `src/apis/<api>.js` file. The glob finds it. No shared file changes.
